...
import org.gravy.prover.princess.PrincessProverFactory;
...
/*
...

		{
			...
			final ProverExpr interpolant =
			    p.interpolate(new int[][] { new int[] { 0 },
			                                 new int[] { 1 } })[0];
			...
			System.out.println(p.substitute(interpolant,
			                                new ProverExpr[] { b },
			                                new ProverExpr[] { a }));
		}
...
//                    ProverExpr[] explanations = ((PrincessProver)p).explainSimply(axiom, hypo, toIgnore);
//
//                    System.out.println("There were " + explanations.length + " abductions");
...
//                    }
	...
	private static ProverExpr getVar(String name, ProverType type, Map<String, ProverExpr> m, Prover p){
		...
		return var;
                ...
                /*
                    final ProverExpr n0 = p.mkVariable("n0", p.getIntType());
                    final ProverExpr n1 = p.mkVariable("n1", p.getIntType());
                    final ProverExpr m0 = p.mkVariable("m0", p.getIntType());
                    final ProverExpr m1 = p.mkVariable("m1", p.getIntType());
                    final ProverExpr zero = p.mkLiteral(0);
                    final ProverExpr one = p.mkLiteral(1);
                    final ProverExpr ten = p.mkLiteral(10);

                    List<ProverExpr> unrolled = new ArrayList<ProverExpr>();
                    
                	int loops = 1;
                    for(int i = 0; i < loops; i++){
                    	ProverExpr m = p.mkVariable("m" + i, p.getIntType());
                    	ProverExpr mNext = p.mkVariable("m" + (i+1), p.getIntType());
                    	ProverExpr n = p.mkVariable("n" + i, p.getIntType());
                    	ProverExpr nNext = p.mkVariable("n" + (i+1), p.getIntType());
                    	ProverExpr ands[] = {
                        		p.mkLeq(m,p.mkLiteral(10)),
                        		p.mkEq(nNext,p.mkPlus(n, one)),
                        		p.mkEq(mNext, p.mkPlus(m,one)),
                        	};
                    	unrolled.add(p.mkAnd(ands));
                    }
                    
                    final ProverExpr finalM = p.mkVariable("m" + loops, p.getIntType());
                    final ProverExpr finalN = p.mkVariable("n" + loops, p.getIntType());

                    unrolled.add(p.mkGt(finalM, ten));

                    
                    final ProverExpr left = p.mkAnd(unrolled.toArray(new ProverExpr[unrolled.size()]));
                    final ProverExpr right = p.mkNot(p.mkEq(finalN,p.mkLiteral(0)));
;
                    		
                    System.out.println("Tying to explain: " + right);
                    System.out.println("Given: " + left);

                   ProverExpr[] toAvoid = {finalN,finalM};
                   for(ProverExpr e : toAvoid){
                   	System.out.println("To avoid: " + e);
                   }

                    //ProverExpr[] explanations = ((PrincessProver)p).explain(left, right);

                    ProverExpr[] explanations = ((PrincessProver)p).explainSimply(left, right,toAvoid);
                    System.out.println("Size of explanation array is: " + explanations.length);
                    for(ProverExpr e : explanations){
                    	System.out.println("Explanation: " + e);
                    }
                    if(false){
                    p.setPartitionNumber(0);
                    //p.addAssertion(explanations[0]);
                    p.addAssertion(p.mkNot(p.mkEq(n0, p.mkLiteral(-1))));	
                    
                    p.setPartitionNumber(1);
                    p.addAssertion(left);

                    p.setPartitionNumber(2);
                    p.addAssertion(p.mkNot(right));
                    System.out.println("Checking the implication " + p.checkSat(true));
                    ProverExpr[] interpolants =
                    		p.interpolate(new int[][] { new int[] { 0 },
        				                                 new int[] { 1 } });
        			...
        			interpolants =
        				    p.interpolate(new int[][] { new int[] { 1 },
        				                                 new int[] { 2 } });
        			for (ProverExpr e : interpolants){
                    ...
                    final ProverExpr n0 = p.mkVariable("n0", p.getIntType());
                    final ProverExpr n1 = p.mkVariable("n1", p.getIntType());
                    final ProverExpr n1NotZero = p.mkNot(p.mkEq(n1,p.mkLiteral(0)));
                    final ProverExpr m0 = p.mkVariable("m0", p.getIntType());
                    final ProverExpr m1 = p.mkVariable("m1", p.getIntType());
                    final ProverExpr zero = p.mkLiteral(0);
                    final ProverExpr one = p.mkLiteral(1);
                    ...
                    final ProverExpr ands[] = {
                    		p.mkLeq(m0,p.mkLiteral(10)),
                    		p.mkEq(n1,p.mkPlus(n0, one)),
                    		p.mkEq(m1, p.mkPlus(m0,one)),
                    		p.mkGt(m1, ten),
                    	};
                    final ProverExpr left = p.mkAnd(ands);
                    final ProverExpr right = n1NotZero;
                    		
                    System.out.println("Tying to explain: " + right);
                    System.out.println("Given: " + left);

                    ProverExpr[] toAvoid = {n1,m1};
                    for(ProverExpr e : toAvoid){
                    	System.out.println("To avoid: " + e);
                    }
                    //ProverExpr[] explanations = ((PrincessProver)p).explain(left, right);
