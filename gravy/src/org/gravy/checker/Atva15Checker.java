...
//		}
		...
		return new Atva15Report(this.cff, atr, feasibleBlocks, fwd_dangerous, bwd_dangerous, unreachableBlocks);
...
//	private int TIME_LIMIT = 8000; // millisecs
	private int TIME_LIMIT = 10; // millisecs
...
//					 return;
	...
	private void coverHardToGetBlocks(Atva15TransitionRelation tr,
			Set<BasicBlock> covered, int phase) {
		HashSet<BasicBlock> result = new HashSet<BasicBlock>(covered);
		...
		Set<BasicBlock> notCovered = new HashSet<BasicBlock>(tr
				.getReachabilityVariables().keySet());
...
//		}
		...
		Atva15JodChecker jodcheck = new Atva15JodChecker(cff, procedure, prover);
		Atva15TransitionRelation jtr = new Atva15TransitionRelation(procedure,
				cff, prover);

		result.addAll(jodcheck.computeJodCover(prover, jtr, covered, phase));
		this.feasibleBlocks = result;
		
		this.dangerousBlocks = jodcheck.dangerousBlocks;
		this.feasibleBlocks = jodcheck.coveredBlocks;
	}
