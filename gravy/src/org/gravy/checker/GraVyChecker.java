...
public class GraVyChecker extends
	...
	public GraVyChecker(AbstractControlFlowFactory cff,
		...
		//assume the Requires clauses.
		for (CfgExpression req : p.getRequires()) {
			p.getRootNode().addStatement(new CfgAssumeStatement(p.getLocation(), req), true);
		}
		//we do not assert the Ensures clauses here because we
		//want them to be generated using our error model instead of
		//assertions.
		
		
		
		
		//TODO: do the unwinding differently
...
//							prover.getBooleanType()));
		...
		{//only reporting stuff in this blocks
	        ...
	        long firstchecktime = firstcheck.getTime();
	        long allchecktime = allchecks.getTime();
	        DEBUG_FIRST_CHECK_TIME+=firstchecktime;
	        DEBUG_ALL_CHECK_TIME+=allchecktime;
		}
	...
	static public long DEBUG_FIRST_CHECK_TIME = 0;
	static public long DEBUG_ALL_CHECK_TIME = 0;
